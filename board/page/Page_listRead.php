<form action="Page_Writetemplate.php" method="post">
    <?php
    ...
    $find = "select * from dy_board where board_pid = 0 order by reg_date desc limit $limitNum, 5";
    ...
    echo "<input type='text' id='find'><input type='button' onclick='findText()' value='∮'><br>";
    $num_row = mysqli_num_rows(mysqli_query($db_con, 'select * from dy_board'));
...
//뒷페이지가 없을때
    ...
    if (isset($_SESSION['id'])) {
        echo "<script> 
            ...
            location.href='http://localhost/Page_findText.php?find='+ findText.value + '&numofpage='+$numofpage;
             }</script>";
    }
    ?>
</form>
...
<script>
    ...
    function zzz(arg) {
        location.href = "http://localhost/Page_listRead.php?numofpage=" + arg;
    }
    function move(arg) {
        if (arg == 1) {
            ...
            location.href = 'http://localhost/Page_resultprint.php';
        }
